...
FastAPI transport, a slot-filling conversation, and three pure calculators.
Python 3.10–3.13, five dependencies, no LLM SDK.

```bash
python3 -m venv ../.venv && source ../.venv/bin/activate
pip install -r requirements.txt

cp .env.example .env          # Gemini key
...
`ollama serve` running. Without either the server still starts and the page still
loads — the first message says what is missing instead of showing a stack trace.

> **Gemini free tier: 20 requests/day, per model.** A turn costs one to three, so
> a free key is good for ~7 turns before returning 429. The bot reports that as a
> spent allowance and keeps every value you gave it. `GEMINI_MODEL` switches model
> when one model's daily 20 is gone — the quota is per project *per model*, so a
> new key on the same project inherits the same exhausted allowance.
...
`(1+r)²` — ₹19,067.62 against a ₹10,00,000 goal at 12% over 10 years.
**Implemented verbatim**, because it is the client's stated requirement, with two
tests pinning it: one asserts the spec value, one asserts the ratio against the
annuity-due value, so the deviation is demonstrably understood rather than
transcribed. Full reasoning in `calculators/sip.py`.
...
```
  app.py         routes, session id, serialization. No logic, no maths, no prompts.
      │
      ▼
  chat/          intent routing, slot-filling state machine, LLM adapter.
      ...
      ▼          └──────────────────┬───────────────────────┘
  calculators/  ◄───────────────────┘
                 pure functions. Deterministic. The only place maths exists.
```
...
both are structural assertions in `tests/test_guards.py`, not conventions.

| File | Holds |
| --- | --- |
| `calculators/rates.py` | `monthly_rate()` = `(1 + R/100)^(1/12) − 1`, defined exactly once in the repo |
| `calculators/validation.py` | the amount/rate/period guards, shared so they cannot drift |
| `calculators/errors.py` | the exception hierarchy, carrying structured fields rather than rendered strings |
...
| `chat/formatting.py` | the number/text boundary, both directions. The only opinion on how money looks |
| `chat/tools.py` | the transport seam: `DirectTools` or `McpTools` |
...
`requirements.txt` carries no LLM dependency as a result, and the tests mock one
function rather than a client library.
...
- **A digression is not a state transition.** "What is an EMI?" mid-flow is a
  message the session answers, not a place it goes. `state`, `slots` and
  `pending_slot` are untouched.
...
Captured output, abridged. Ollama `qwen2.5:7b-instruct`, because Gemini's free
tier ran out mid-capture.
...
constants rather than restated. The SIP tool also carries the annuity-due warning,
what it costs, and how to recover the conventional figure — a caller checking the
arithmetic from outside has nowhere else to learn it.
...
the exception to a string across a pipe would take all of that away and still look
like it worked. So the wire carries the structure:
...
Both work from here or from the repository root. They need **no API key, no
network, and no environment variable** — the model is stubbed, so the suite runs
from a fresh clone. Python 3.10 through 3.13.

- Every numbered case in `TEST_VECTORS.md` — L1–L11, S1–S7, W1–W10, G1–G5 and the
  degenerate-path vectors in §6. Those values were committed *before* any
...
- The committed frontend build in `ui/`: no external URL in any `src`, `href` or
  `@import`, every reference resolving to a file that exists. Deliberately a
  Python test — `ui/` is the one directory nobody writes by hand, so a CDN
  arriving via a dependency shows up only in the bundle, and this is the suite
  that runs without a toolchain.
